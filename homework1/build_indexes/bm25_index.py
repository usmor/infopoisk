...
class BM25IndexLib(BaseIndex):
    ...
    def build(self, texts: List[str]):
        ...
        """
        self.corpus = texts
        # Токенизируем корпус, используя регулярные выражения
        self.tokenized_corpus = [re.findall(r"\w+|[^\w\s]", text) for text in texts]
        # Инициализируем BM25 retriever с токенизированным корпусом и заданными параметрами
    ...
    def search(self, query: str, top_k: int = 5):
        ...
        # Получаем оценки релевантности для всех документов в корпусе
...
class BM25IndexDict(BaseIndex):
    ...
    def __init__(self, k1=1.5, b=0.75):
        self.inverted_index = {}
        self.doc_lens = {}
        self.avg_dl = 0  # средняя длина документа для BM25 формулы
        self.num_docs = 0
        self.k1 = k1
        self.b = b
    ...
    def build(self, texts: List[str]):
        ...
        for doc_id, text in enumerate(texts):
            ...
            # и подсчитываем частоту каждого токена в документе
        ...
        # Вычисляем среднюю длину документа для BM25 формулы
    ...
    def search(self, query: str, top_k: int = 5):
        # Токенизируем запрос, используя регулярные выражения и 
        # инициализируем словарь для хранения счетов релевантности
        ...
        for token in tokens:
            ...
            # Получаем список документов и частот для токена и количество документов, содержащих токен
            doc_list = self.inverted_index[token]
            doc_freq = len(doc_list)

            # Вычисляем IDF для токена с учетом сглаживания
            idf = np.log((N - doc_freq + 0.5) / (doc_freq + 0.5) + 1)

            # Обновляем счетчики релевантности для документов, содержащих токен, 
            # используя формулу BM25
            for doc_id, tf in doc_list.items():
                dl = self.doc_lens[doc_id]
                
                # Вычисляем числитель и знаменатель для формулы BM25
                numerator = tf * (k1 + 1)
                denominator = tf + k1 * (1 - b + b * dl / avg_dl)

                score = idf * numerator / denominator

                # Обновляем счет релевантности для документа, 
                # суммируя вклады от всех токенов запроса
...
class BM25IndexMatrix(BaseIndex):
    ...
    def build(self, texts: List[str]):
        ...
        # Сохраняем длины документов для BM25 формулы 
        # и вычисляем среднюю длину документа
        self.doc_lens = np.array([len(toks) for toks in tokenized])
        self.avg_dl = np.mean(self.doc_lens)

        # Создаем словарь термин -> индекс строки
        self.term_to_idx = {
        ...
        # Заполняем разреженную матрицу term-document, 
        # подсчитывая частоту каждого токена в каждом документе
